...
public:
	...
	void LocalFile_SaveStart(std::wstring sPath = L"", int nType = -1)
	{
		...
		if (m_pLocalFileLocker)
		{
			if (m_pLocalFileLocker->GetFileLocked() != m_oConverterFromEditor.m_oInfo.m_sFileSrc)
			{
				...
				if (nType != -1)
					isSaveToEditFormat = LocalFileSaveEnd_CheckSaveAs();

				if (isSaveToEditFormat)
					RELEASEOBJECT(m_pLocalFileLocker);
			}
	...
	bool LocalFileSaveEnd_CheckSaveAs();
...
public:
	...
	bool CheckPopup(std::wstring sUrl, bool bIsBeforeBrowse = false, bool bIsBackground = false, bool bIsNotOpenLinks = false, const std::wstring& sFrameUrl = L"")
	{
		...
		if (sUrlLower.find(L"filehandler?action=download") != std::wstring::npos)
			...
			bIsDownload     = true;
		else if (sUrlLower.find(L"filehandler?action=bulk") != std::wstring::npos)
			bIsDownload     = true;
							  ...
							  CefRefPtr<CefBeforeDownloadCallback> callback) OVERRIDE
{
	...
	NSEditorApi::CAscSaveDialog* pData = new NSEditorApi::CAscSaveDialog();
	pData->put_Id(m_pParent->GetId());
	std::wstring sSuggestName = suggested_name.ToWString();
	if (0 == sSuggestName.find(L"localhost-") && sSuggestName.length() > 10)
		sSuggestName = sSuggestName.substr(10);
	pData->put_FilePath(sSuggestName);
...
bool CCefView_Private::LocalFileSaveEnd_CheckSaveAs()
{
...
void CCefView_Private::LocalFile_SaveEnd(int nError, const std::wstring& sPass)
{
	...
	bool bIsSavedFileCurrent = LocalFileSaveEnd_CheckSaveAs();
	...
	if (ASC_CONSTANT_CANCEL_SAVE == nError || !bIsSavedFileCurrent)
	{
		...
		message->GetArgumentList()->SetInt(1, (ASC_CONSTANT_CANCEL_SAVE == nError || 0 == nError) ? 1 : 2);
...
void CCefView::Apply(NSEditorApi::CAscMenuEvent* pEvent)
{
	...
	if (!browser)
	{
		...
		{
			if (!m_pInternal->m_bIsDestroying)
			{
				m_pInternal->m_bIsDestroying = true;
				if (m_pInternal->m_pManager)
					m_pInternal->m_pManager->OnDestroyWindow();

				if (!m_pInternal->m_bIsLoaded)
				{
					NSEditorApi::CAscCefMenuEvent* pEvent = CreateCefEvent(ASC_MENU_EVENT_TYPE_CEF_DESTROYWINDOW);
