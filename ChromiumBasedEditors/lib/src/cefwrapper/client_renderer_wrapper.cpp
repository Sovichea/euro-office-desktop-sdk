...
namespace NSCommon
{
	...
	public:
		...
		const
			...
			inline void
			reset(Type* pPointer = NULL)
		{
			*this = pPointer;
		}
	};

	int GetEncryptedVersion()
	{
		return 2;
	}
...
if (window.DocsAPI && window.DocsAPI.DocEditor) \n\
	...
	return undefined; \n\
				...
				if (strUrl.find(L"sdk/Common/AllFonts.js") != std::wstring::npos || strUrl.find(L"sdkjs/common/AllFonts.js") != std::wstring::npos)
				{
			...
			else if (name == "setCookie")
			{
				...
				++iter;
				message->GetArgumentList()->SetString(2, (*iter)->GetStringValue());
				++iter;
				message->GetArgumentList()->SetString(3, (*iter)->GetStringValue());
				++iter;
				message->GetArgumentList()->SetString(4, (*iter)->GetStringValue());
				++iter;
				SEND_MESSAGE_TO_BROWSER_PROCESS(message);
...
if (main.DisableVersionHistory) main.DisableVersionHistory(); \
			...
			else if (name == "Print_Start")
			{
				...
				message->GetArgumentList()->SetString(0, (*iter)->GetStringValue());
				++iter;
				message->GetArgumentList()->SetInt(1, (*iter)->GetIntValue());
				++iter;
			...
			else if (name == "Print_Page")
			{
				...
				message->GetArgumentList()->SetDouble(2, (*iter)->GetDoubleValue());
				++iter;
				message->GetArgumentList()->SetDouble(3, (*iter)->GetDoubleValue());
				++iter;
				SEND_MESSAGE_TO_BROWSER_PROCESS(message);
...
#define IMAGE_CHECKER_SIZE 50
			...
			else if (name == "execCommand")
			{
						...
						if (NULL == savedInfo && tmpInfo)
						{
							...
							else
							{
								...
								if (nServerPrivateKeyVersion == 2)
									NSOpenSSL::AES_Decrypt_desktop_GCM(
										U_TO_UTF8(tmpInfo->m_sPassword), privateKeyEnc, privateKey, CAscRendererProcessParams::getInstance().GetProperty("user"), nServerPrivateKeyVersionOffset);
								else
								...
								if (nServerPrivateKeyVersion < NSCommon::GetEncryptedVersion())
								{
									...
									std::string sCode =
										("setTimeout(function() { window.cloudCryptoCommand && window.cloudCryptoCommand(\"updateEncryptionKeys\", { publicKey : \"" + publicKey +
										 "\", privateKeyEnc : \"" + privateEnc + "\" }); }, 10);");
									CefRefPtr<CefFrame> _frame = CefV8Context::GetCurrentContext()->GetFrame();
									_frame->ExecuteJavaScript(sCode, _frame->GetURL(), 0);
								}
							}
				...
				else if ("portal:checkpwd" == sCommand)
				{
					...
					CefRefPtr<CefV8Value> retval;
					CefRefPtr<CefV8Exception> exception;
					bool bValid = CefV8Context::GetCurrentContext()->Eval(
						"(function() { return document.getElementById(\"" + emailId + "\").value; })();",
#ifndef CEF_2623
						"", 0,
#endif
						retval, exception);
					...
					bValid = CefV8Context::GetCurrentContext()->Eval(
						"(function() { return document.getElementById(\"" + passwordId + "\").value; })();",
#ifndef CEF_2623
						"", 0,
#endif
						retval, exception);
...
window._external_process_callback = {};\n\
			...
			else if (name == "_AddAudio")
			{
				...
				std::wstring sCode =
					L"(function(){ window.on_add_multimedia_local(\"" + sImage + L".png\", \"" + sImage + L"." + sExt + L"\"); window.on_add_multimedia_local = undefined; })();"; // .svg
				CefRefPtr<CefFrame> _frame = CefV8Context::GetCurrentContext()->GetFrame();
				_frame->ExecuteJavaScript(sCode, _frame->GetURL(), 0);
				return true;
			}
...
if (window.onSystemMessage2) window.onSystemMessage2(e);\n\
			...
			else if (name == "CompareDocumentUrl" || name == "CompareDocumentFile" || name == "MergeDocumentUrl" || name == "MergeDocumentFile")
			{
...
window.AscDesktopEditor.CallInFrame(\"" +
									sId + "\", \
			...
			else if (name == "CheckCloudFeatures")
			{
				...
				if (!bIsStopPaydDesktopFeatures)
				{
					...
					if (bIsChanged)
					{
						...
						CefRefPtr<CefV8Exception> exception;
						bool bValid = CefV8Context::GetCurrentContext()->Eval(
							"window.AscDesktopEditor.CallInAllWindows(\
\"function(){ window.onfeaturesavailable && window.onfeaturesavailable(" +
								oAddonsChecker.GetFeaturesJSArray() + "); }\"\
);",
#ifndef CEF_2623
							"", 0,
#endif
				...
				if (m_sEditorsCloudFeatures.empty())
				{
					...
					if (!sCloudCryptoId.empty())
					{
						...
						CefRefPtr<CefV8Exception> exception;
						bool bValid = CefV8Context::GetCurrentContext()->Eval(
							"(function(){var _editor = window.Asc.editor ? window.Asc.editor : window.editor; _editor && _editor.asc_pluginStop && _editor.asc_pluginStop(\"" + sCloudCryptoId +
								"\");})();",
#ifndef CEF_2623
							"", 0,
#endif
...
#define SCALES_COUNT 13
			...
			else if (name == "getDictionariesPath")
			{
				if (IsLocalFile(true))
...
\n\
window.AscDesktopEditor.InitJSContext();",
					curFrame->GetURL(), 0);
...
return this.split(str).join(newStr);\
		...
		virtual void OnFocusedNodeChanged(CefRefPtr<client::ClientAppRenderer> app, CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame, CefRefPtr<CefDOMNode> node) OVERRIDE
		{
	  ...
	  browser->SendProcessMessage(PID_BROWSER, message);
		...
		virtual bool OnProcessMessageReceived(
			CefRefPtr<client::ClientAppRenderer> app, CefRefPtr<CefBrowser> browser,
#ifndef MESSAGE_IN_BROWSER
...
if (window.Asc && window.Asc.editor && !window.editor) window.Asc.editor.asc_nativePrint(undefined, undefined);\
			...
			else if (sMessageName == "onlocaldocument_loadend")
			{
				...
				if (_frame)
				{
					...
					std::string sCode =
						"window.AscDesktopEditor.LocalFileRecoverFolder(\"" + U_TO_UTF8(sFolderJS) + "\");window.AscDesktopEditor.LocalFileSetSourcePath(\"" + U_TO_UTF8(sFileSrc) + "\");";
					_frame->ExecuteJavaScript(sCode, _frame->GetURL(), 0);
					...
					{
						_frame->ExecuteJavaScript(
							"(function(){var _editor = window[\"editor\"]; if (!_editor && window[\"Asc\"]) _editor = window[\"Asc\"][\"editor\"]; if (_editor && _editor.asc_setLocalRestrictions) "
							"_editor.asc_setLocalRestrictions(" +
								std::to_string(bIsLockedFile) + ", true);})();",
							_frame->GetURL(), 0);
					}
			...
			else if (sMessageName == "oncompare_loadend" || sMessageName == "onmerge_loadend")
			{
				CefRefPtr<CefFrame> _frame = GetEditorFrame(browser);
				if (!_frame)
					_frame = browser->GetMainFrame();
			...
			else if (sMessageName == "native_viewer_onopened")
			{
				...
				if (_frame)
				{
					...
					std::string sCode = "window.AscDesktopEditor.NativeViewerOpen(\"" + U_TO_UTF8(s1) + "\", \"" + U_TO_UTF8(s2) + "\", \"" + U_TO_UTF8(s3) + "\");";
			...
			else if (sMessageName == "on_signature_defaultcertificate_ret" || sMessageName == "on_signature_selectsertificate_ret" || sMessageName == "on_signature_viewcertificate_ret")
			{
...
_editor && _editor.local_sendEvent && _editor.local_sendEvent(\"asc_onError\", \"" +
										sWarning + "\", 0);})();";
					_frame->ExecuteJavaScript(sCode, _frame->GetURL(), 0);
				}
				return true;
...
{ window.Asc.plugin.onSystemMessage(_arg); } \n\
			...
			else if (sMessageName == "onload_crypt_document")
			{
				...
				if (_frame)
				{
					...
					std::string sCode =
						("window.AscDesktopEditor.SetCryptDocumentFolder(\"" + sDirectoryRecoverA + "\");\n\
window.AscDesktopEditor.loadLocalFile(\"" +
						 sFilePath + "\", function(data) {\n\
				...
				if (_frame)
					_frame->ExecuteJavaScript(
						"(function() { if (!window.on_get_advanced_encrypted_data) return; window.on_get_advanced_encrypted_data(\"" + sRet +
							"\"); delete window.on_get_advanced_encrypted_data; })();",
						_frame->GetURL(), 0);
			}
				...
				if (_frame)
					_frame->ExecuteJavaScript(
						"(function() { if (!window.on_set_advanced_encrypted_data) return; window.on_set_advanced_encrypted_data(\"" + sRet +
							"\"); delete window.on_set_advanced_encrypted_data; })();",
						_frame->GetURL(), 0);
			}
